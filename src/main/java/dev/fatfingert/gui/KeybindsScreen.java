...
public class KeybindsScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        g.text(this.font, "KEYBINDS", textX, cardY + 12, FatTheme.TEXT, true);
        String sub = capturing != null
                ? "press a key · Esc unbinds · click to cancel"
                : "click a key to change it";
        g.text(this.font, this.font.plainSubstrByWidth(sub, cardX + cardW - PAD - textX),
                textX, cardY + 24, capturing != null ? FatTheme.AMBER : FatTheme.TEXT_DIM, false);

        // Row stripes and labels sit under the key buttons, so draw them first.
        int rowRight = rowRight();
        int end = Math.min(bindings.size(), scrollOffset + itemsPerPage);
        for (int i = scrollOffset; i < end; i++) {
            KeyMapping mapping = bindings.get(i).mapping();
            int rowY = listY + 2 + (i - scrollOffset) * ROW_H;
            boolean waiting = mapping == capturing;
            ...
            g.text(this.font, this.font.plainSubstrByWidth(name, room), innerX + 8, rowY + 7,
                    waiting ? FatTheme.AMBER : FatTheme.TEXT, false);
        }
        ...
        g.text(this.font, this.font.plainSubstrByWidth("also under Options › Controls", cardX + cardW - PAD - hintX),
